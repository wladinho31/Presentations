...
resource "azurerm_public_ip" "vm_pip" {
  name                = "${var.vm_name}-${local.env_name}-pip"
...
resource "azurerm_network_interface" "vm_nic" {
  name                = "${var.vm_name}-${local.env_name}-nic"
  location            = data.azurerm_resource_group.rg.location
  resource_group_name = data.azurerm_resource_group.rg.name
  tags                = var.tags
...
resource "azurerm_linux_virtual_machine" "vm" {
  ...
  name                  = "${var.vm_name}-${local.env_name}"
  location              = data.azurerm_resource_group.rg.location
  resource_group_name   = data.azurerm_resource_group.rg.name
  ...
  os_disk {
    name                 = "${var.vm_name}-${local.env_name}-OsDisk"
    caching              = "ReadWrite"
  ...
  computer_name                   = "${var.vm_name}-${local.env_name}"
...
resource "null_resource" "deploy_nginx" {
  ...
  provisioner "file" {
    source      = var.source_code[terraform.workspace]
    destination = "."
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "sudo cp -r ${var.source_code[terraform.workspace]}/* /var/www/html"
    ]
